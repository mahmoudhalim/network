...
class Node(TypedDict):
    netmask: str
    if_count:  NotRequired[int]
    if_prefix: NotRequired[int]
    connections: NotRequired[dict[str, Connection]]


RoutersDict = dict[str, Node]
...
def ipv4_to_value(ipv4_addr: str) -> int:
    ...
    """
    ip_num = list(map(int, ipv4_addr.split(".")))
    ip = 0
    shift = 24
    for i in ip_num:
        ip += i << shift
...
def get_subnet_mask_value(slash: str) -> int:
    ...
    assert 0 <= subnet <= 32, "subnet mask must be between 1 and 32"
    return ((1 << subnet) - 1) << (32-subnet)
...
def ips_same_subnet(ip1: str, ip2: str, slash: str) -> bool:
    ...
    return ip1_int & subnet == ip2_int & subnet
...
def find_router_for_ip(routers: RoutersDict, ip: str):
    """
    ...
    for router_ip, router_data in routers.items():
        if ips_same_subnet(ip, router_ip, router_data['netmask']):
            return router_ip
...
#     print(value_to_ipv4(ipv4_to_value("192.168.1.1")))
#     print(get_subnet_mask_value("/16"))
#     print(get_subnet_mask_value("192.168.1.1/16"))
#     print(ips_same_subnet("192.168.1.5", "192.168.1.10", "/24"))
#     print(ips_same_subnet("192.168.1.5", "192.168.99.10", "/16"))
#     print(ips_same_subnet("192.168.1.5", "191.167.1.10", "/8"))
#     print(ips_same_subnet("192.168.1.5", "191.167.1.10", "/8"))
#     print(f"{get_network(0x01020304, 0xffffff00):x}")
#     routers= {
#         "1.2.3.1": {
#             "netmask": "/24"
#         },
...
def read_routers(file_name: str):
...
def print_routers(routers: RoutersDict):
    print("Routers:")
...
def print_same_subnets(src_dest_pairs: list[list[str]]):
    print("IP Pairs:")
...
def print_ip_routers(routers: RoutersDict, src_dest_pairs: list[list[str]]):
    print("Routers and corresponding IPs:")

    all_ips = sorted(set([i for pair in src_dest_pairs for i in pair]))

    router_host_map: dict[str, list[str]] = {}
...
def main(argv: list[str]):
    if "my_tests" in globals() and callable(my_tests): # pyright: ignore[reportUnknownArgumentType, reportUndefinedVariable]
        my_tests() # type: ignore
        return 0
